...
def build_sales_by_region(df: DataFrame) -> DataFrame:
    ...
    return (
        ...
        .agg(
            ...
            F.sum(
                F.when(F.col("is_discounted"), F.col("net_revenue")).otherwise(0)
            ).alias("discounted_revenue"),
        )
...
def build_customer_segments(df: DataFrame) -> DataFrame:
    ...
    rfm = (
        ...
        .withColumn("recency_days",
            F.datediff(F.lit(latest_date), F.col("last_order_date")))
    )

    # Quintile scoring (1=best, 5=worst for R; 1=worst, 5=best for F/M)
    tile = 5
    ...
    rfm = (
        rfm
        .withColumn("r_score", F.ntile(tile).over(r_window))
